...
export async function GET() {
  try {
    console.log('[API] /api/about GET called');
    
    let aboutPage = await prisma.aboutPage.findFirst();

    if (!aboutPage) {
      console.log('[API] /api/about: No about page found, creating default');
    ...
    console.log('[API] /api/about returning data');
    return successResponse(parsedAboutPage);
  } catch (error: any) {
    console.error('[API] Error fetching about page:', error);
