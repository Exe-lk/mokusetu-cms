...
import { createService, updateService, fetchServices } from '@/src/store/slices/servicesSlice';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (service?.id) {
        ...
        resetForm();
        dispatch(fetchServices()); // Refresh the services table
        onClose();
      }
    } catch (error) {
      ...
      Swal.fire({
        title: 'Error',
        text: 'Error saving service. Please try again.',
        icon: 'error',
        confirmButtonText: 'OK',
      });
    } finally {
